...
class SpotifyRecommender:
    def __init__(self, csv_path: str):
        ...
        # Check if data exists, if not we'll handle gracefully
        if os.path.exists(csv_path):
            self.load_data()
        else:
            print(f"Warning: Dataset not found at {csv_path}. Recommender will return empty results.")
    ...
    def recommend_similar_songs(self, song_id: str, top_n: int = 20):
        if self.df is None or self.song_matrix is None: return []
